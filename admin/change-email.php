...
if (isset($_POST['reset-password'])) {
    ...
    if (!empty($email) && !empty($password) && !empty($new_email)) {
        $checkMail = "SELECT email FROM admin WHERE email = '$email' AND password = '$password'";
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            $updateToken = "UPDATE admin SET tocken = '$reset_key' WHERE email = '$email'";
            $resUpdate = mysqli_query($conn, $updateToken);
            if ($resUpdate) {
                $to = $new_email;
                $from = 'example@gmail.com';
                $fromName = 'Vehicle Stories';
                $subject = "Admin Email Reset Email";
                $htmlContent = '
              <!DOCTYPE html>
  <html lang="en">
  <head>
  ...
  <body>
      <div class="box">
          ...
          <a href="http://localhost/Vehicle-Stories/admin/process-email-change.php?reset-link=' . $reset_key . '">Varify Now</a>
      </div>
  </body>
  </html>';
                ...
                <div class="container d-flex justify-content-center align-items-center p-0 m-0">
                    <div class="card border-top border-0 border-4 border-dark" style="max-width: 450px;width: 100%;">
                        ...
                        <div class="col-12 my-3">
                            <div class="d-grid">
                                <button type="submit" id="reset-email" name="reset-password" class="btn btn-dark btn-lg px-5"><i class="bx bxs-lock-open"></i>Change Email</button>
                            </div>
                        </div>
                        <hr>
                        </form>
